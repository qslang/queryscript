...
mod tests {
    ...
    fn test_schemas() {
            ...
            fs::read_dir(&tests).expect(format!("Could not read {}", tests.display()).as_str())
        {
            ...
            if !entry
                .file_type()
                .expect(format!("Could not read file type for {:?}", entry).as_str())
                .is_dir()
            {
                continue;
            }
    ...
    fn test_double_schemas() {
        let prefix = Path::new(env!("CARGO_MANIFEST_DIR")).join("tests/qs/jaffle/");

        let compiler = compile::Compiler::new().expect("Failed to create compiler");

        for fname in ["staging.qs", "queries.qs"].iter() {
