...
let key_assumptions =
    select
        ...
        ent_ae_logos,
        ent_ae_new_arr
    from marketing_gsheets.fy22_key_assumptions
;
...
let web_traffic =
    ...
    left join user_agent_registry.user_agent_registry AS user_agent_registry ON coalesce(daily_website_traffic.context_useragent, daily_website_traffic.context_user_agent) = user_agent_registry.context_useragent
    where split_part(
        regexp_substr(
            case
...
fn breakdown_by_month(date_field, metric_name) {
	let filtered_leads = select * from lead
	...
	select date_trunc(month, date_field) as month, count(distinct lower(email)) as metric_name
    from filtered_leads
    group by 1
}


let lead_creation = breakdown_by_month(createddate, count_lead_creation);
let mql = breakdown_by_month(first_mql_date__c, count_mql);
let reengaged_mql = breakdown_by_month(most_recent_reengaged_date__c, count_reengaged_mql);
let sal = breakdown_by_month(sal_date__c, count_sql);
...
-- Ideally you could define a function that takes the segment name as input and it returns the pipeline, logo, and ARR creation numbers
-- for it, but the use of LIKE vs. = is inconsistet across the two segments.
pipeline_created as (
    select
...
marketing_funnel as (
    select
		...
		-- It would be nice to be able to metaprogram these suffixes, e.g. something like
		--     select * as *_assumption from key_assumptions
		-- to automate this
		)
